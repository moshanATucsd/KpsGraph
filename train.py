...
def train(epoch, best_val_loss):
    ...
    for batch_idx, (data, relations) in enumerate(train_loader):
        ...
        data, data_err, relations = Variable(data),Variable(data_err), Variable(relations)
        optimizer.zero_grad()
        if args.cuda:
            data_err,data, relations = data_err.cuda(), data.cuda(), relations.cuda()

        logits = encoder(data_err, rel_rec, rel_send)
        #edges = gumbel_softmax(logits, tau=args.temp, hard=args.hard)
        edges = my_softmax(logits, -1)

        if args.decoder == 'rnn':
            output = decoder(data_err, edges, rel_rec, rel_send, 100, burn_in=True, burn_in_steps=args.timesteps - args.prediction_steps)
        else:
            output = decoder(data_err, edges, rel_rec, rel_send, args.prediction_steps)
        ...
        if args.prior:
            loss_kl = kl_categorical(prob, log_prior, args.num_kps)
        else:
    ...
    for batch_idx, (data, relations) in enumerate(valid_loader):
        ...
        output = decoder(data_err, edges, rel_rec, rel_send, 1) # validation output uses teacher forcing
...
def test():
    ...
    for batch_idx, (data, relations) in enumerate(test_loader):
        ...
        output = decoder(data_err, edges, rel_rec, rel_send, 1)
