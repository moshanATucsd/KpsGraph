...
parser.add_argument('--suffix', type=str, default='_single12',
                    help='Suffix for training data (e.g. "_cads".')
...
parser.add_argument('--dims', type=int, default=5,
                    help='The number of dimensions (position).')
...
if args.encoder == 'mlp':
    model = InteractionNet(args.dims, args.hidden,
                           args.edge_types,
...
def train(epoch, best_val_accuracy):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        #data=data_all[:,:,0:2]
        data=data[:,:,:args.dims]
        variance=error_values(data,target)
        data_err=data.clone()
